...
// ---------------------------------------------------------------------------
pub use crate::vcp::r#impl::general::utility_function_types::*;
// ---------------------------------------------------------------------------
use maplit::hashmap;
...
// ----------------------------------------------------------------------------
// Utilities below are used by the testing framework and may be useful, directly
// or indirectly for anyone implementing code that uses the platform.  Others
// will migrate here from PlatformAPI because we decided not to expose them as
// "first class citizens" in, e.g, a server that facilitates using the platform
...
pub fn create_accumulators(f: CreateAccumulatorData) -> CreateAccumulators {
    Arc::new(move |n, sdcts| {
        create_for_accumulator_fields(sdcts, |i| f(n + i))
            .map(|m| {
                m.iter()
                    .map(|(k, CreateAccumulatorResponse { new_accum_data, new_accum_value })|
                         (*k, (new_accum_data.clone(), new_accum_value.clone(), hashmap!())))
                    .collect()
            })
    })
}
...
// which uses it in anoter test framework that is not translated (yet?) to Rust
#[allow(dead_code)]
...
pub fn get_accumulator_public_data_from_map() -> GetAccumulatorPublicDataFromMap {
    Arc::new(|acc_hdls, idx| {
        let (AccumulatorData { public_data: pub_data, .. }, _, _) =
             acc_hdls.get(&idx).ok_or(Error::General(format!(
...
pub fn update_accumulator_witness_with_map(
    ...
    a_idx: CredAttrIndex,
    vals: &[DataValue],
    adui_api: &AccumWitnessUpdateInfo,
    sn: AccumulatorBatchSeqNo
) -> VCPResult<()> {
