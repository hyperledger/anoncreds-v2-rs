# to create a markdown file with a table of contents:
# - M-x org-md-export-to-markdown
# - make fix-underscores-in-vcp-readme-markdown
...
- By accessing the functionality via a Swagger/OpenAPI interface. We have previously built an HTTP
  ...
  this is internal work, and is not up-to-date.  Future work may include doing similar in Rust and
  making it available in this repo.
...
the API and test its behaviour in various scenarios.  Tests are expressed in JSON and automatically
run by our framework.  A new JSON test can be added to
[[../../tests/data/JSON/TestSequences/LicenseSubscription]], where there are a number of examples
...
[[../../tests/vcp/impl/general/utility_function_types.rs]].)
...
[[../../tests/data/JSON/TestSequences/LicenseSubscription/json_test_028_example_single_issuer_and_credential_in_accum_no_update.json]]
...
Although using the API directly would require creating and initialising ~AccumulatorData~ for each
such field, the second (~CreateAccumulators)~ step creates and initialises an accumulator for each
~CTAccumulatorMember~ field in the schema (only one in our example).

In the third (~Sign~) step, Issuer ~DMV~ signs a credential for ~Holder1~, specifying values for each of
...
*** CreateAccumulators
**** Effects
  - Creates ~AccumulatorDdata~ for each ~CTAccumulatorMember~ attribute in specified Issuer's schema
**** Arguments
   - ~IssuerLabel~
**** API method(s) invoked
...
**** Effects
  ...
  - Stores "update information" associated with updating ~Accumulatormembershipwitness~ es from previous ~AccumulatorBatchSeqNo~ to new one, for use by subsequent ~UpdateAccumulatorWitness~ steps
**** Arguments
   - ~IssuerLabel~
...
**** Effects
  ...
  - This is possible only if
    - a) specified Holder already has an ~AccumulatorMembershipWitness~ for identified accumulator for
      an ~AccumulatorBatchSeqNo~ that is at most the target
    ~AccumulatorBatchSeqNo~, and
    - b) there have been sufficient ~AccumulatorAddRemove~ steps performed that "update information"
      has been stored to enable updating to specifed ~AccumulatorBatchSeqNo~.
...
**** Comments
  ...
  - If it did, Holders could always regenerate discarded ~AccumulatorMembershipWitness~ es *provided*
    ...
    have to request a ~AccumulatorMembershipWitness~ from the relevant Revocation Manager; the test
...
**** Effects
  ...
  - note that there is no step for creating a ~MembershipProvingKey~ because one is automatically
...
**** Arguments
   ...
   - ~IssuerLabel~
   - ~CredAttrIndex~
   - ~Word64~: the minimum value in the range
   - ~Word64~: the maximum value in the range
**** Comments
...
**** Arguments
   ...
   - ~[(IssuerLabel, CredAttrIndex)]~: a list of attributes in other credentals required to be equal to specified attribute
**** Comments
...
**** Effects
  ...
  - An error is generated if specified Holder cannot satisfy preiously added requirements because,
...
**** Arguments
   ...
   - ~CreateVerifyExpectation~: expected outcome for attempt to create and then verify a proof
     consistent with established requiremment.  Possible values are currently:
...
- although the test has SLOW or SLOWSLOW in its name, we know that it is ~NotSoSlow~ with the specific underlying library
- the test is known to ~Fail~, because of a known bug in the cryptography library, for example
- we want to ~Skip~ the test for some reason

We would like to improve the override system.  In the meantime, it is documented in
[[../../generate-tests-from-json/src/lib.rs]].
...
- specific
  - functions called from general that implement "primitive" crypto features (e.g., sign, prove, verify) for a specific underlying cryptography library (so far, only AC2C)

** General

A proof is created from
- =SignatureAndRelatedData= : signature from an issuer on a list =DataValue=
...
- =DataForVerifier= : a proof (created by =create_proof=)
...
That proof wrapped up into an opaque =DataForVerifier= API type.
...
#+begin_example
├── interfaces
    ├── crypto_interface.rs               : functions that a specific crypto library needs to implement
    ├── mod.rs
...
├── impl
    ...
    ├── common
    │   ├── mod.rs
    │   └── types.rs                      : data declarations used by all specific implementations
    ├── general
    │   ├── mod.rs
...
The directory structure for the anoncreds-v2-rs implementation is:

#+begin_example
├── impl
    ├── ac2c                              : anoncreds-v2-rs implementation of CryptoInterface
        |
        ├── accumulator.rs                : AC2C VB implementation of CryptoInterface accumulator primitives
        |
        ├── impl_ac2c.rs                  : instantiates the CryptoInterface with AC2C implementation of primitives
        ├── mod.rs
...
([[./interfaces/crypto_interface.rs]]) struct with "pointers" to the AC2C implementation of
[[./interfaces/primitives.rs]].  That initialized struct is referenced as =CRYPTO_INTERFACE_AC2C=.
...
The =specific_prover=, =specific_verifier=, =specific_verify_decryption= values are first pass to
the non primitive, =create_proof=, =verify_proof=, and =verify_decryption= functions to create a higher
level platform function that is then assigned to their associated fields.

An example of making this connection can be seen in the =run_json_test_ac2c= function in
[[../../tests/vcp/impl/general/testing_framework/utils.rs]].  (Ignore the body of =run_json_test_ac2c=.)

# --------------------------------------------------
** Creating an issuer's public and secret data (e.g., keys)

To prepare for signing credentials, an Issuer uses =create_signer_data= in =PlatformApi= ([[./api.rs]]).

The type of that function, =CreateSignerData=, is defined in [[./interfaces/primitives.rs]].

It takes
- a =Natural= (an RNG seed)
- and a list of =ClaimType= (both defined in [[./interfaces/types.rs]])
...
then =create_signer_data= (in [[./impl/ac2c/signer.rs]]) is invoked.
...
=SignerData= ([[./interfaces/types.rs]]) contains
- =SignerSecretData=
  - an opaque representation of the AC2C secret data
- =SignerPublicData=
  - an opaque representation of the AC2C public data, and
...
To sign credentials, an Issuer uses =PlatformApi= ([[./api.rs]]) =sign=
of type =Sign= ([[./interfaces/primitives.rs]]).

It takes
- a =Natural= (an RNG seed)
- a list of =DataValue= ([[./interfaces/types.rs]])
- =SignerData= (from =create_signer_data= above)

The AC2C implementation of =sign= is in [[./impl/ac2c/signer.rs]].
...
The general =create_proof= function ([[./impl/general/proof.rs]]) takes
- proof requirements : =HashMap<CredentialLabel, CredentialReqs>=
  - =CredentialLabel=
    - an identifier used when establishing equalities between different credentials;
      and for referencing signatures of a credential
  - =CredentialReqs= ([[./interfaces/types.rs]])
...
- signatures, etc : =HashMap<CredentialLabel, SignatureAndRelatedData>=
  - =CredentialLabel=
    - a locally unique label associating a signature with a particular =CredentialReqs=
  - =SignatureAndRelatedData= contains
    - =Signature=
      - used to create a proof-of-knowledge
    - list of =DataValue=
      - that values that produced the signature
...
- transforms human-friendly =CredentialReqs= into machine-friendly "proof instructions" and equality requirements
  - via =presentation_request_setup= ([[./impl/general/presentation_request_setup.rs]])
- validates the =CredentialReqs= against schemas
- calls the specific crypto library function =specific_prover=

The AC2C =specific_prover= (named =specific_prover_ac2c= in [[./impl/ac2c/proof.rs]])
- creates an AC2C =Presentation= (i.e., "proof") ([[../presentation.rs]])
...
Like the general =create_proof= function, the general =verify_proof= function ([[./impl/general/proof.rs]]) takes
- proof requirements : =HashMap<CredentialLabel, CredentialReqs>=
- shared parameters : =HashMap<SharedParamKey, SharedParamValue>=
- =Option<Nonce>=

It also takes a =DataForVerifier= produced from =create_proof=.
...
The AC2C =specific_verifier= (named =specific_verifier_ac2c= in [[./impl/ac2c/proof.rs]])
convert the VCP information and data into formats used by AC2C then calls
=anoncreds-v2-rs= =Presentation:verify= to verify the proof.

# --------------------------------------------------
** Proofs with revealed values

Values to be revealed are specified in the =disclosed: Disclosed= field of =CredentialReqs=.

=Disclosed= ([[./interfaces/types.rs]]) is a list of indices into the list of =DataValue= that were signed,
specifying which values should be disclosed.

Both the general =create_proof= and =verify_proof= functions ([[./impl/general/proof.rs]]) call
=presentation_request_setup= ([[./impl/general/presentation_request_setup.rs]]) that calls
...
in the AC2C case, =specific_prover_ac2c= and =specific_verifier_ac2c= ([[./impl/ac2c/proof.rs]])

The specific functions call =presentation_schema_from= which calls
=transform_instruction= ([[./impl/ac2c/presentation_request_setup.rs]]) to
...
=SupportedDisclosure= is then given to =generate_statements= ([[./impl/ac2c/presentation_request_setup.rs]]).
...
- =anoncreds-v2-rs= =IndexMap<CredentialLabel, PresentationCredential>=
  - created by a call to =presentation_credentials_from= ([[./impl/ac2c/proof.rs]])
...
=InRange= ([[./interfaces/types.rs]]) is a list of =InRangeInfo= that contain
...
=EncryptedFor= ([[./interfaces/types.rs]]) is a list of =IndexAndLabel= that contain
...
=EqualTo= ([[./interfaces/types.rs]]) is a list of =EqInfo= that contain
...
=presentation_schema_from= ([[./impl/ac2c/presentation_request_setup.rs]]) with those ~EqualityReqs~.
...
=InAccum= ([[./interfaces/types.rs]]) is a list of =InAccumInfo= that contain
...
There are functions for ([[./api.rs]], [[./interfaces/primitives.rs]])
...
The AC2C versions are in [[./impl/ac2c/accumulator.rs]].
