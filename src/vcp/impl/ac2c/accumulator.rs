...
pub fn create_accumulator_data() -> CreateAccumulatorData {
    Arc::new(|rng_seed| {
        ...
        let ad  = AccumulatorData { public_data: to_api(pk)?, secret_data: to_api(sk)?, };
        Ok(CreateAccumulatorResponse { new_accum_data : ad, new_accum_value : to_api(acc)? })
    })
}
...
pub fn accumulator_add_remove() -> AccumulatorAddRemove {
    Arc::new(|ad, acc, adds, rms| {
        ...
        let mut wits_for_new : HashMap<HolderID, AccumulatorMembershipWitness> = HashMap::new();
        for (hid, el) in adds {
            let e   = from_api(el)?;
            let wit = vb20::MembershipWitness::new(e, acc2, &sk);
            wits_for_new.insert(hid.clone(), to_api(wit)?);
        }
        ...
        let r = AccumulatorAddRemoveResponse {
            ...
            wits_for_new,
            updated_accum_data  : ad.clone(),
            updated_accum_value : to_api(acc2)?,
        };
        Ok(r)
    })
}
