use super::{ExpandedPublicKey, PokSignatureProof};
...
fn pok_sig_proof_works() {
    ...
    let epk = ExpandedPublicKey::new(pk, NonZeroUsize::new(4).unwrap());

    let res = PokSignature::commit(signature, &epk, &proof_messages, &mut rng);
    assert!(res.is_ok());
    ...
    assert!(proof.verify(rvl_msgs, &epk).is_ok());

    let mut transcript = Transcript::new(b"signature proof of knowledge");
    proof.add_proof_contribution(&epk, rvl_msgs, challenge, &mut transcript);
    transcript.append_message(b"nonce", nonce.to_be_bytes().as_ref());
    transcript.challenge_bytes(b"signature proof of knowledge", &mut tmp);
